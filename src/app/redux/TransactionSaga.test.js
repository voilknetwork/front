...
import voilk, { api, broadcast } from 'voilk';
...
describe('TransactionSaga', () => {
    ...
    describe('recoverAccount', () => {
        ...
        it('should call getWithdrawRoutes with account name and outgoing as parameters', () => {
            const noAutoCoins = gen.clone();
            const actual = noAutoCoins.next().value;
            const mockCall = call(
                [api, api.getWithdrawRoutes],
                ['one', 'outgoing']
            );
            expect(actual).toEqual(mockCall);
            const done = noAutoCoins.next().done;
        ...
        it('should call getWithdrawRoutes with account name and outgoing as parameters, and reset all outgoing auto coining routes to 0.', () => {
            const withAutoCoins = gen.clone();
            withAutoCoins.next([{ from_account: 'one', to_account: 'two' }])
                .value;
            const actual = withAutoCoins.next([
            ...
            const mockCall = all([
                call(
                    ...
                    [
                        'SHR7UbRctdfcdBU6rMBEX5yPjWaR68xmq6buCkotR7RVEJHYWt1Jb',
                        'one',
                        'two',
                        0,
                        true,
