            ...
            for (const [type /*, operation*/] of operations) {
                switch (type) {
                    case 'vote':
                        ...
                        if (/Voting weight is too small/.test(errorStr)) {
                            ...
                            errorStr =
                                'Voting weight is too small, please accumulate more voting power or voilk power.';
                        }
                        break;
                    case 'comment':
                        if (
                    ...
                    case 'transfer':
                        ...
                        break;
                    case 'withdraw_coining':
                        if (
                            /Account registered by another account requires 10x account creation fee worth of Voilk Power/.test(
                                errorStr
                            )
                        )
                            errorKey =
                                'Account requires 10x the account creation fee in Voilk Power (approximately 30 SP) before it can power down.';
                        break;
                    default:
                        break;
                }
                if (state.hasIn(['TransactionError', type + '_listener'])) {
                    ...
                    if (error.message) {
                        ...
                        // https://github.com/voilknetwork/voilk.com/issues/222
                    ...
                    if (/unknown key: /.test(errorKey)) {
                        errorKey = "Voilk account doesn't exist.";
                        errorStr =
                            "Transaction failed: Voilk account doesn't exist.";
                    }
