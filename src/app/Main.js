...
function runApp(initial_state) {
    ...
    const cmd = command => {
        ...
        switch (command) {
            ...
            case CMD_LOG_T:
                ...
                if (konami.enabled) {
                    voilk.api.setOptions({ logger: console });
                } else {
                    voilk.api.setOptions({ logger: false });
                }
    ...
    voilk.config.set('address_prefix', config.address_prefix);
    voilk.config.set('chain_id', config.chain_id);
    window.$SHR_Config = config;
    plugins(config);
    if (initial_state.offchain.serverBusy) {
        window.$SHR_ServerBusy = true;
    }
    if (initial_state.offchain.csrf) {
        window.$SHR_csrf = initial_state.offchain.csrf;
