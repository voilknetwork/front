...
class Market extends React.Component {
    ...
    componentWillReceiveProps(np) {
        if (!this.props.ticker && np.ticker) {
            ...
            if (this.refs.buyVoilk_price)
                this.refs.buyVoilk_price.value = parseFloat(lowest_ask).toFixed(
                    6
                );
            if (this.refs.sellVoilk_price)
                this.refs.sellVoilk_price.value = parseFloat(
                    highest_bid
                ).toFixed(6);
        }
    }
    ...
    shouldComponentUpdate = (nextProps, nextState) => {
        ...
        let tc =
            ...
            this.props.ticker.vsd_volume !== nextProps.ticker.vsd_volume;
    ...
    buyVoilk = e => {
        ...
        const amount_to_sell = parseFloat(
            ReactDOM.findDOMNode(this.refs.buyVoilk_total).value
        );
        const min_to_receive = parseFloat(
            ReactDOM.findDOMNode(this.refs.buyVoilk_amount).value
        );
    ...
    sellVoilk = e => {
        ...
        if (!user) return;
        const min_to_receive = parseFloat(
            ReactDOM.findDOMNode(this.refs.sellVoilk_total).value
        );
        const amount_to_sell = parseFloat(
            ReactDOM.findDOMNode(this.refs.sellVoilk_amount).value
        );
    ...
    setFormPrice = price => {
        ...
        this.refs.sellVoilk_price.value = p.toFixed(6);
        this.refs.buyVoilk_price.value = p.toFixed(6);

        const samount = parseFloat(this.refs.sellVoilk_amount.value);
        if (samount >= 0)
            this.refs.sellVoilk_total.value = roundDown(p * samount, 3);

        const bamount = parseFloat(this.refs.buyVoilk_amount.value);
        if (bamount >= 0)
            this.refs.buyVoilk_total.value = roundUp(p * bamount, 3);

        this.validateBuyVoilk();
        this.validateSellVoilk();
    };
    ...
    validateBuyVoilk = () => {
        const amount = parseFloat(this.refs.buyVoilk_amount.value);
        const price = parseFloat(this.refs.buyVoilk_price.value);
        const total = parseFloat(this.refs.buyVoilk_total.value);
    ...
    validateSellVoilk = () => {
        const amount = parseFloat(this.refs.sellVoilk_amount.value);
        const price = parseFloat(this.refs.sellVoilk_price.value);
        const total = parseFloat(this.refs.sellVoilk_total.value);
        const valid = amount > 0 && price > 0 && total > 0;
        const { highest_bid } = this.props.ticker;
        this.setState({
            sell_disabled: !valid,
    ...
    render() {
        ...
        if (typeof this.props.ticker != 'undefined') {
            ...
            ticker = {
                ...
                vsd_volume: parseFloat(vsd_volume),
                feed_price:
            ...
            return ['bids', 'asks'].reduce((out, side) => {
                ...
                orders[side].map(o => {
                    ...
                    if (
                        ...
                        // buff[buff.length-1].voilk += o.voilk
                        // buff[buff.length-1].vsd   += o.vsd
                        // buff[buff.length-1].vsd_depth = o.vsd_depth
                        // buff[buff.length-1].voilk_depth = o.voilk_depth
                    } else {
                        buff.push(o);
                    }
                    last = o;
            ...
            const rows =
                ...
                open_orders.map(o => (
                    <tr key={o.orderid}>
                        ...
                        <td>{o.voilk}</td>
                        <td>{o.vsd.replace('VSD', DEBT_TOKEN_SHORT)}</td>
                        <td>
                            <a
                                href="#"
                    ...
                    <thead>
                        <tr>
                            ...
                            <th
                                ...
                                onClick={e => handleToggleOpenOrdersSort('vsd')}
                            >
            ...
            <div>
                ...
                <div className="row">
                    <div className="small-12 medium-6 columns">
                        ...
                        <form className="Market__orderform" onSubmit={buyVoilk}>
                            <div className="row">
                                        ...
                                        <input
                                            ...
                                            onChange={e => {
                                                const amount = parseFloat(
                                                    this.refs.buyVoilk_amount
                                                        .value
                                                );
                                                const price = parseFloat(
                                                    this.refs.buyVoilk_price
                                                        .value
                                                );
                                                if (amount >= 0 && price >= 0)
                                                    this.refs.buyVoilk_total.value = roundUp(
                                                        price * amount,
                                                        3
                                                    );
                                                validateBuyVoilk();
                                        ...
                                        <input
                                            ...
                                            onChange={e => {
                                                ...
                                                const total = parseFloat(
                                                    this.refs.buyVoilk_total
                                                        .value
                                                );
                                                if (total >= 0 && price >= 0)
                                                    this.refs.buyVoilk_amount.value = roundUp(
                                                        total / price,
                                                        3
                                                    );
                                                validateBuyVoilk();
                                            ...
                                            <small>
                                                <a
                                                    ...
                                                    onClick={e => {
                                                        ...
                                                        const total = account.vsd_balance.split(
                                                            ' '
                                                        )[0];
                                                        this.refs.buyVoilk_total.value = total;
                                                        if (price >= 0)
                                                            this.refs.buyVoilk_amount.value = roundDown(
                                                                parseFloat(
                                                                    total
                                                                ) / price,
                                                                3
                                        ...
                                        <small>
                                            <a
                                                ...
                                                onClick={e => {
                                                    ...
                                                    this.refs.buyVoilk_price.value =
                                                        ticker.lowest_ask;
                                                    if (amount >= 0)
                                                        this.refs.buyVoilk_total.value = roundUp(
                                                            amount * price,
                                ...
                                <div className="column small-9 large-8">
                                    <div className="input-group">
                                        <input
                                            ...
                                            onChange={e => {
                                                const amount = parseFloat(
                                                    this.refs.sellVoilk_amount
                                                        .value
                                                );
                                                const price = parseFloat(
                                                    this.refs.sellVoilk_price
                                                        .value
                                                );
                                                if (amount >= 0 && price >= 0)
                                                    this.refs.sellVoilk_total.value = roundDown(
                                                        price * amount,
                                                        3
                                                    );
                                                validateSellVoilk();
                                    ...
                                    <div className="input-group">
                                        <input
                                            ...
                                            onChange={e => {
                                                ...
                                                const total = parseFloat(
                                                    this.refs.sellVoilk_total
                                                        .value
                                                );
                                                if (price >= 0 && total >= 0)
                                                    this.refs.sellVoilk_amount.value = roundUp(
                                                        total / price,
                                                        3
                                                    );
                                                validateSellVoilk();
                                            ...
                                            <small>
                                                <a
                                                    ...
                                                    onClick={e => {
                                                        ...
                                                        this.refs.sellVoilk_amount.value = amount;
                                                        if (price >= 0)
                                                            this.refs.sellVoilk_total.value = roundDown(
                                                                price *
                                                                    parseFloat(
                                        ...
                                        <small>
                                            <a
                                                ...
                                                onClick={e => {
                                                    ...
                                                    const price =
                                                        ticker.highest_bid;
                                                    this.refs.sellVoilk_price.value = price;
                                                    if (amount >= 0)
                                                        this.refs.sellVoilk_total.value = roundDown(
                                                            parseFloat(price) *
                                                                amount,
    ...
    component: connect(
        ...
        dispatch => ({
            ...
            placeOrder: (
                ...
                // create_order jsc 12345 "1.000 VSD" "100.000 VOILK" true 1467122240 false
