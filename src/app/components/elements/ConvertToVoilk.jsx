...
class ConvertToVoilk extends React.Component {
    constructor() {
        super();
        this.state = {};
    }
    ...
    shouldComponentUpdate = shouldComponentUpdate(this, 'ConvertToVoilk');
    dispatchSubmit = () => {
            ...
            >
                <div className="row">
                    <div className="small-12 columns">
                        <h1>
                            {tt('converttovoilk_jsx.convert_to_LIQUID_TOKEN', {
                                LIQUID_TOKEN,
                            })}
                        </h1>
                        <p>
                            {tt(
                                'converttovoilk_jsx.DEBT_TOKEN_will_be_unavailable',
                                { DEBT_TOKEN }
                            )}
                        </p>
                        <p>
                            {tt(
                                'converttovoilk_jsx.your_existing_DEBT_TOKEN_are_liquid_and_transferable',
                                { link: tt('g.buy_or_sell'), DEBT_TOKEN }
                            )}
                        </p>
                        <p>
                            {tt(
                                'converttovoilk_jsx.this_is_a_price_feed_conversion'
                            )}
                        </p>
                    </div>
                </div>
...
export default reduxForm(
    { form: 'convertToVoilk', fields: ['amount'] },
    // mapStateToProps
    (state, ownProps) => {
        const current = state.user.get('current');
        const username = current.get('username');
        const account = state.global.getIn(['accounts', username]);
        const vsd_balance = account.get('vsd_balance');
        const max = vsd_balance.split(' ')[0];
                    ...
                    successCallback: () => {
                        ...
                        dispatch(
                            appActions.addNotification({
                                key: 'convert_sd_to_voilk_' + Date.now(),
