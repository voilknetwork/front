...
import { PublicKey, Signature, hash } from 'voilk/lib/auth/ecc';
import { api, broadcast } from 'voilk';
...
export default function useGeneralApi(app) {
    ...
    router.post('/accounts', koaBody, function*() {
        ...
        try {
            ...
            //     throw new Error("Only one Voilk account per user is allowed in order to prevent abuse");
            // }
            ...
            if (same_ip_account) {
                ...
                if (minutes < 10) {
                    ...
                    throw new Error(
                        'Only one Voilk account allowed per IP address every 10 minutes'
                    );
                }
            }
