...
import { PrivateKey, PublicKey } from 'voilk/lib/auth/ecc';
import { api, broadcast, auth, memo } from 'voilk';
...
function* broadcastPayload({
    ...
    if ($SHR_Config.read_only_mode) return;
...
function* accepted_comment({ operation }) {
    ...
    // update again with new $$ amount from the voilkd node
...
function* accepted_withdraw_coining({ operation }) {
...
export function* preBroadcast_comment({ operation, username }) {
    ...
    // TODO Slightly smaller blockchain comments: if body === json_metadata.voilk.link && Object.keys(voilk).length > 1 remove voilk.link ..This requires an adjust of get_state and the API refresh of the comment to put the voilk.link back if Object.keys(voilk).length >= 1
    ...
    if (comment_options) {
        const {
            ...
            percent_voilk_dollars = 10000, // 10000 === 100%
            allow_votes = true,
...
export function* createPermlink(title, author, parent_author, parent_permlink) {
    ...
    if (permlink.length > 255) {
        // VOILKNETWORK_MAX_PERMLINK_LENGTH
...
export function* recoverAccount({
    ...
    try {
        // TODO: Incoinigate wrapping in a redux-saga call fn, so it can be tested!.
        ...
        // Reset all outgoing auto-coining routes for this user. Condenser - #2835
        const outgoingAutoCoiningRoutes = yield call(
            [api, api.getWithdrawRoutes],
            [account.name, 'outgoing']
        );
        if (outgoingAutoCoiningRoutes && outgoingAutoCoiningRoutes.length > 0) {
            yield all(
                outgoingAutoCoiningRoutes.map(ovr => {
                    return call(
                        [broadcast, broadcast.setWithdrawCoiningRoute],
