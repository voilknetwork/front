...
live voilk.com site and it is the most supported (and fastest) method of
...
```bash
docker run -it -p 8080:8080 voilknetwork/condenser
```

Environment variables can be added like this:

```bash
docker run -it --env SDC_DATABASE_URL="mysql://user:pass@hostname/databasename" -p 8080:8080 voilknetwork/condenser
```
...
```bash
git clone https://github.com/voilknetwork/condenser
cd condenser
...
`wss://voilkd.steeemit.com`. This is actually on the real blockchain and
...
```bash
export SDC_CLIENT_VOILKD_URL="wss://voilkd.voilk.com"
export SDC_SERVER_VOILKD_URL="wss://voilkd.voilk.com"
```
...
```bash
export SDC_DATABASE_URL="mysql://root:password@127.0.0.1/voilknetwork_dev"
```
...
Now launch mysql client and create voilknetwork_dev database:
```bash
mysql -u root
> create database voilknetwork_dev;
> quit
```
...
```
yarn build
OFFLINE_SSR_TEST=true SDC_DATABASE_URL="mysql://root@127.0.0.1/voilknetwork_dev" NODE_ENV=production node --prof lib/server/index.js
```
...
To run a Selenium test suite, start the condenser docker image with a name `condenser` (like `docker run --name condenser -itp 8080:8080 voilknetwork/condenser:latest`) and then run the blackboxtest image attached to the condneser image's network:

```
docker build -t=voilknetwork/condenser-blackboxtest blackboxtest/
docker run --network container:condenser voilknetwork/condenser-blackboxtest:latest
...
If you find a security issue please report details to: security@voilk.com
