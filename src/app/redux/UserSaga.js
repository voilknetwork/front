...
import { PrivateKey, Signature, hash } from 'voilk/lib/auth/ecc';
...
// const isHighSecurityOperations = ['transfer', 'transfer_to_coining', 'withdraw_coining',
    ...
    if (!operationType || saveLogin) {
        ...
        yield put(
            userActions.setUser({
                ...
                private_keys,
                login_owner_pubkey,
                coining_shares: account.get('coining_shares'),
                received_coining_shares: account.get('received_coining_shares'),
                delegated_coining_shares: account.get(
...
function* uploadImage({
    ...
    const sig = Signature.signBufferSha256(bufSha, d);
    const postUrl = `${$SHR_Config.upload_image}/${username}/${sig.toHex()}`;
