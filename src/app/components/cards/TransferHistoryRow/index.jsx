        ...
        if (type === 'transfer_to_coining') {
            ...
            if (data.from === context) {
                if (data.to === '') {
                    ...
                    // tt('g.transfer') + amount + tt('g.to') + 'SHARE POWER';
                } else {
                    message = (
                        <span>
                            {tt(
                    ...
                    // tt('g.transfer') + amount + ' SHARE POWER' + tt('g.to');
                }
            } else if (data.to === context) {
                message = (
                    <span>
                        {tt(
                            'transferhistoryrow_jsx.transfer_to_coining.to_self',
                            { amount }
                        )}
                        {otherAccountLink(data.from)}
                    </span>
                );
                // tt('g.receive') + amount + ' SHARE POWER' + tt('g.from');
            } else {
                message = (
                    <span>
                        {tt(
                ...
                // tt('g.transfer') + amount + ' SHARE POWER' + tt('g.from') +data.from + tt('g.to');
            }
        } else if (
            ...
            // `${curation_reward} SHARE POWER` + tt('g.for');
            ...
            // `${data.vsd_payout}${voilk_payout}, ${tt( 'g.and' )} ${author_reward} SHARE POWER ${tt('g.for')}`;
            ...
            if (parseFloat(data.reward_coins.split(' ')[0]) > 0)
                rewards.push(`${reward_coins} SHARE POWER`);
            ...
            // `${benefactor_reward} SHARE POWER for ${ data.author }/${data.permlink}`;
        } else {
