    ...
    connect(
        ...
        dispatch => ({
            ...
            setMetaData: (id, jsonMetadata) => {
                dispatch(
                    globalActions.setMetaData({
                        id,
                        meta: jsonMetadata ? jsonMetadata.voilk : null,
                    })
                );
            },
            ...
            reply: ({
                ...
                meta.app = 'voilknetwork/0.1';
                if (isStory) {
                    meta.format = isHtml ? 'html' : 'markdown';
                }

                // if(Object.keys(json_metadata.voilk).length === 0) json_metadata = {}// keep json_metadata minimal
                    ...
                    switch (payoutType) {
                        case '0%': // decline payout
                            __config.comment_options = {
                                max_accepted_payout: '0.000 VSD',
                            };
                            break;
                        case '100%': // 100% voilk power payout
                            __config.comment_options = {
                                percent_voilk_dollars: 0, // 10000 === 100% (of 50%)
                            };
                            break;
                        default: // 50% voilk power, 50% sd+voilk
                    }
                }
