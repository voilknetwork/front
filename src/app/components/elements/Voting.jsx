...
import { pricePerVoilk } from 'app/utils/StateFunctions';
...
const VOTE_WEIGHT_DROPDOWN_THRESHOLD = 1.0 * 500.0;
const VSD_PRINT_RATE_MAX = 10000;
...
class Voting extends React.Component {
    static propTypes = {
        ...
        price_per_voilk: PropTypes.number,
        vsd_print_rate: PropTypes.number,
    };
	...
	let display_pending_payout = pending_payout/multiplier;
        ...
        const percent_voilk_dollars =
            post_obj.get('percent_voilk_dollars') / 20000;
	...
	let total_author_payout = parsePayoutAmount(
        ...
        let total_curator_payout = parsePayoutAmount(
        ...
        if (cashout_active) {
            ...
            if (max_payout > 0) {
                payoutItems.push({
                    value:
                        '(' +
                        formatDecimal(pending_payout_printed_vsd*10).join('') +
                        ' ' +
                        DEBT_TOKEN_SHORT +
                        ', ' +
        ...
        if (promoted > 0) {
            payoutItems.push({
                value: tt('voting_jsx.promotion_cost', {
                    value: formatDecimal(display_promoted).join(''),
                }),
            });
        }
        if (total_author_payout > 0) {
            payoutItems.push({
                value: tt('voting_jsx.past_payouts', {
                    value: formatDecimal(
                        display_author_payout + display_curator_payout
                    ).join(''),
                }),
            });
            payoutItems.push({
                value: tt('voting_jsx.past_payouts_author', {
                    value: formatDecimal(display_author_payout).join(''),
                }),
            });
            payoutItems.push({
                value: tt('voting_jsx.past_payouts_curators', {
                    value: formatDecimal(display_curator_payout).join(''),
                }),
            });
        }
        const payoutEl = (
...
export default connect(
    ...
    (state, ownProps) => {
        ...
        const username = current_account
            ...
            : null;
        const coining_shares = current_account
            ? current_account.get('coining_shares')
            : 0.0;
        const delegated_coining_shares = current_account
            ? current_account.get('delegated_coining_shares')
            : 0.0;
        const received_coining_shares = current_account
            ? current_account.get('received_coining_shares')
            : 0.0;
        const net_coining_shares =
        ...
        const vsd_print_rate = state.global.getIn(['props', 'vsd_print_rate']);
        
	const enable_slider =
            net_coining_shares > VOTE_WEIGHT_DROPDOWN_THRESHOLD;
