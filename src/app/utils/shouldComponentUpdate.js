...
export default function(instance, name) {
    ...
    if (
        process.env.BROWSER &&
        window.voilkDebug_shouldComponentUpdate === undefined
    ) {
        window.voilkDebug_shouldComponentUpdate = false; // console command line completion
    }
    return (nextProps, nextState) => {
        const upd = mixin(nextProps, nextState);
        // Usage: voilkDebug_shouldComponentUpdate = true
        // Or: voilkDebug_shouldComponentUpdate = /Comment/
        if (
            upd &&
            process.env.BROWSER &&
            window.voilkDebug_shouldComponentUpdate
        ) {
            const filter = window.voilkDebug_shouldComponentUpdate;
            if (filter.test) {
