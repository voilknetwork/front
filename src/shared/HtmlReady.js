 ...
 *    - basic sanitization?
 *  2. Voilk.com Rendering - add in proprietary Voilk.com functions/links
...
function link(state, child) {
    ...
    if (url) {
        state.links.add(url);
        if (state.mutate) {
            // If this link is not relative, http, https, or voilk -- add https.
            if (!/^((#)|(\/(?!\/))|(((voilk|https?):)?\/\/))/.test(url)) {
            ...
            if (
                (url.indexOf('#') !== 0 && // Allow in-page links
                    (child.textContent.match(/(www\.)?voilknetwork\.com/i) &&
                        !url.match(
                            /https?:\/\/(.*@)?(www\.)?voilknetwork\.com/i
                        ))) ||
...
// For all img elements with non-local URLs, prepend the proxy URL (e.g. `https://img0.voilk.com/0x0/`)
...
function ipfsPrefix(url) {
    if ($SHR_Config.ipfs_prefix) {
        // Convert //ipfs/xxx  or /ipfs/xxx  into  https://voilk.com/ipfs/xxxxx
        if (/^\/?\/ipfs\//.test(url)) {
            const slash = url.charAt(1) === '/' ? 1 : 0;
            url = url.substring(slash + '/ipfs/'.length); // start with only 1 /
            return $SHR_Config.ipfs_prefix + '/' + url;
        }
    }
    return url;
}
