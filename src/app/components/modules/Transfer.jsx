...
import { APP_NAME, LIQUID_TOKEN, COINING_TOKEN } from 'app/client_config';
...
class TransferForm extends Component {
    static propTypes = {
        ...
        toCoining: PropTypes.bool.isRequired,
        ...
        const insufficientFunds = (asset, amount) => {
            ...
            const balanceValue =
                !asset || asset === 'VOILK'
                    ...
                    : asset === 'VSD'
                      ? isWithdraw
                        ? currentAccount.get('savings_vsd_balance')
                        : currentAccount.get('vsd_balance')
                      : null;
            if (!balanceValue) return false;
            const balance = balanceValue.split(' ')[0];
            return parseFloat(amount) > parseFloat(balance);
        };
        const { toCoining } = props;
        const fields = toCoining ? ['to', 'amount'] : ['to', 'amount', 'asset'];
        if (
            !toCoining &&
    ...
    balanceValue() {
        ...
        return !asset || asset.value === 'VOILK'
            ? isWithdraw
              ? currentAccount.get('savings_balance')
              : currentAccount.get('balance')
            : asset.value === 'VSD'
    ...
    render() {
        ...
        const powerTip3 = tt(
            'tips_js.converted_COINING_TOKEN_can_be_sent_to_yourself_but_can_not_transfer_again',
            { LIQUID_TOKEN, COINING_TOKEN }
        );
                        ...
                        >
                            ...
                            {asset && (
                                ...
                                >
                                    ...
                                    >
                                        <option value="VOILK">VOILK</option>
        ...
        return (
            <div>
                <div className="row">
                    <h3 className="column">
                        {toCoining
                            ? tt('transfer_jsx.convert_to_COINING_TOKEN', {
                                  COINING_TOKEN,
                              })
                            : transferType}
                    </h3>
...
export default connect(
    ...
    (state, ownProps) => {
        ...
        const toCoining = initialValues.asset === 'COINS';
        ...
        if (!toCoining && !initialValues.transferType)
    ...
    dispatch => ({
        dispatchSubmit: ({
            ...
            if (
                ...
                throw new Error(
                    `Invalid transfer params: toCoining ${
                        toCoining
            ...
            const asset2 = toCoining ? 'VOILK' : asset;
            const operation = {
                from: username,
                to,
                amount: parseFloat(amount, 10).toFixed(3) + ' ' + asset2,
                memo: toCoining ? undefined : memo ? memo : '',
            };
